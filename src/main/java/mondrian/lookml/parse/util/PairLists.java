...
import static mondrian.lookml.parse.util.Nullness.castNonNull;
import static mondrian.lookml.parse.util.Nullness.castNonNullArray;
import static mondrian.lookml.parse.util.Nullness.castNonNullList;
...
class PairLists {
  ...
  static class ArrayImmutablePairList<T, U>
    ...
    @SuppressWarnings("unchecked")
    @Override public <R> List<R> transform(BiFunction<T, U, R> function) {
      final int size = elements.length / 2;
      return new AbstractList<R>() {
        @Override public int size() {
          return size;
        }
